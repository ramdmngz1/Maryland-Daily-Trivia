...
final class AppAttestManager {
    ...
    private func performDebugAttestation() async throws {
        ...
        var request = URLRequest(url: APIEnvironment.url(path: "auth/attest"))
    ...
    private func refreshAccessToken() async throws {
        ...
        var request = URLRequest(url: APIEnvironment.url(path: "auth/refresh"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        ...
        if let newRefresh = result.refreshToken {
            KeychainHelper.save(key: refreshTokenKey, value: newRefresh)
        }
    ...
    private func fetchChallenge(deviceId: String) async throws -> String {
        var components = URLComponents(url: APIEnvironment.url(path: "auth/challenge"), resolvingAgainstBaseURL: false)!
